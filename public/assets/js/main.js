...
function revelarDatos(){
	$('#datos_screen').show();
	$('#historial_screen').hide();
}

function revelarHistorial(){
	$('#datos_screen').hide();
	$('#historial_screen').show();
}

function desactivarAuto(){
    const boxes = document.querySelectorAll('.auto');
    if(document.getElementById('desactivarAuto').checked){
        boxes.forEach(box => {
            box.style.display = '';
        });
    } else {
...
function filtrar_cliente(){
    document.filtrar_clientes.submit();
}

function filtrar_cartera(){
    document.getElementById("filtrar_cartera").submit();
}

window.onload = function() {

	fetch('/datos_clientes', {
		method: 'POST',
		headers: {
	...
	.then(data => {
		var elemento = $("#brow_cli");
		var add = ""
		data.forEach( function(valor, indice, array) {
			// add += "<option value=" + JSON.stringify(valor['cli_anu']) + " id='"+ valor['cli_id'] +"'></option>";
			// elemento.innerHTML=add;
			elemento.append("<option value=" + JSON.stringify(valor['cli_anu']) + " id='"+ valor['cli_id'] +"'></option>");
...
function busqueda_cliente(){
	var datos = document.getElementById('brow_cli');
	var objetivo = document.getElementById('a_buscar').value;
	...
	for (var i = 0; i < datos.options.length; i++) {
		if(datos.options[i].value == objetivo){
			window.location.href = "/clientes_ficha/"+datos.options[i].id;
		}
...
function limpiarFiltros(){
	document.getElementById('iniFecha').value = '';
	document.getElementById('finFecha').value = '';
	// document.getElementById('periodo').value = '';
	document.getElementById('ha_contratado').value = '';
	document.getElementById('acc_com').value = '';
	// document.getElementById('campanya').value = '';
	document.getElementById('localidad').value = '';
	// document.getElementById('sector').value = '';
	document.getElementById('no_contrato').value = '';
}

function limpiarFiltros2(){
	document.getElementById('age').value = '';
	document.getElementById('ultContrato').value = '';
	document.getElementById('ultVisita').value = '';
}


function filtrar_clientes_bajas(){
	document.getElementById("form_clientes_bajas").submit();
}
